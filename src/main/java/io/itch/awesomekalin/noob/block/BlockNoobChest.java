...
public class BlockNoobChest extends ElementsTheGameofNoobs.ModElement {
	...
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("noobchest"));
		elements.items.add(() -> new ItemBlock(block).setRegistryName(block.getRegistryName()));
	}

	@Override
	public void init(FMLInitializationEvent event) {
		GameRegistry.registerTileEntity(TileEntityCustom.class, "noob:tileentitynoobchest");
	}

	@SideOnly(Side.CLIENT)
	@Override
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0, new ModelResourceLocation("noob:noobchest", "inventory"));
	}
	...
	public static class BlockCustom extends Block implements ITileEntityProvider {
		...
		public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random random) {
			...
			if (true)
				for (int l = 0; l < 4; ++l) {
					...
					world.spawnParticle(EnumParticleTypes.SLIME, d0, d1, d2, 0, 0, 0);
				}
		}
		...
		public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer entity, EnumHand hand, EnumFacing direction,
			...
			int z = pos.getZ();
			{
				...
				ProcedureNoobChestOnBlockRightClicked.executeProcedure($_dependencies);
			}
			return true;
		}
	}
	...
	public static class TileEntityCustom extends TileEntityLockableLoot {
		...
		public String getGuiID() {
			return "noob:noobchest";
		}

		@Override
		public Container createContainer(InventoryPlayer playerInventory, EntityPlayer playerIn) {
			return new GuiNoobChestGUI.GuiContainerMod(this.getWorld(), this.getPos().getX(), this.getPos().getY(), this.getPos().getZ(), playerIn);
		}
