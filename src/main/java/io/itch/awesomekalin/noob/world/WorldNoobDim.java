...
package io.itch.awesomekalin.noob.world;
...
import io.itch.awesomekalin.noob.item.ItemNoobDim;
import io.itch.awesomekalin.noob.ElementsTheGameofNoobs;

import com.google.common.cache.LoadingCache;

@ElementsTheGameofNoobs.ModElement.Tag
public class WorldNoobDim extends ElementsTheGameofNoobs.ModElement {
	public static int DIMID = 3;
	public static final boolean NETHER_TYPE = false;
	@GameRegistry.ObjectHolder("noob:noobdim_portal")
	public static final BlockCustomPortal portal = null;
	public static DimensionType dtype;
	public WorldNoobDim(ElementsTheGameofNoobs instance) {
		super(instance, 6);
	}
	...
	public void initElements() {
		...
		elements.items.add(() -> new ItemNoobDim().setUnlocalizedName("noobdim").setRegistryName("noobdim"));
	}
	...
	public void preInit(FMLPreInitializationEvent event) {
		if (DimensionManager.isDimensionRegistered(DIMID)) {
			...
			System.err.println("Dimension ID for dimension noobdim is already registered. Falling back to ID: " + DIMID);
		}
		dtype = DimensionType.register("noobdim", "_noobdim", DIMID, WorldProviderMod.class, true);
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(ItemNoobDim.block, 0, new ModelResourceLocation("noob:noobdim", "inventory"));
	}
					...
					for (int l7 = 1; l7 < 3; ++l7) {
						for (int k8 = -1; k8 < 3; ++k8) {
							...
							this.world.setBlockState(new BlockPos(k9, k10, k11),
									flag ? Blocks.DIRT.getStateFromMeta(0).getBlock().getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			}
				...
				for (int l8 = 0; l8 < 4; ++l8) {
					for (int l9 = -1; l9 < 4; ++l9) {
						...
						this.world.setBlockState(new BlockPos(l10, l11, k12),
								flag1 ? Blocks.DIRT.getStateFromMeta(0).getBlock().getDefaultState() : iblockstate, 2);
					}
				}
	...
	public static class BlockCustomPortal extends BlockPortal {
		public BlockCustomPortal() {
			setHardness(-1.0F);
			setUnlocalizedName("noobdim_portal");
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, EnumFacing p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock())
							|| this.world.getBlockState(blockpos.down()).getBlock() != Blocks.DIRT.getStateFromMeta(0).getBlock()) {
						break;
					}
				}
				Block block = this.world.getBlockState(p_180120_1_.offset(p_180120_2_, i)).getBlock();
				return block == Blocks.DIRT.getStateFromMeta(0).getBlock() ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (block != Blocks.DIRT.getStateFromMeta(0).getBlock()) {
								break label56;
							}
	...
	public static class GenLayerBiomesCustom extends GenLayer {
		private Biome[] allowedBiomes = {Biome.REGISTRY.getObject(new ResourceLocation("noob:noobbio")),};
